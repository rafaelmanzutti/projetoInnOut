...
class WorkingHours extends Model {
    ...
    public function innout($time) {
        ...
        $this->worked_time = getSecondFromDateInterval($this->getWorkedInterval());
        if($this->id) {
            $this->update();
        } else {
            $this->insert();
    ...
    function getExitTime() {
        [$t1,,, $t4] = $this->getTimes();
       // $workday = new DateInterval('PT8H'); *poderia ser assim ou como está abaixo.
    ...
    public static function getMonthlyReport($user_id, $date) {
        ...
        // poderia usar tb o WorkingHours:: no lugar do static:: pq esta dentro do modelo
        $result = static::getResultSetFromSelect([
            'user_id' => $user_id,
            'raw' => "work_date between '{$startDate}' AND '{$endDate}'"
        ]);
